...
def qstring_page(qstring, page_num):
    qdict = QueryDict(qstring, mutable=True)
    return _qdict_set_keys(qdict, "page=%s" % page_num).urlencode()

def qstring_sort(qstring, sort):
    qdict = QueryDict(qstring, mutable=True)
    qdict = _qdict_del_keys(qdict, "page")
    if qdict.get('sort') == sort:
        sort = '-' + sort
    return _qdict_set_keys(qdict, "sort=%s" % sort).urlencode()

register = template.Library()
register.tag(qstring)
register.filter('qstring_del', qstring_del)
register.filter('qstring_set', qstring_set)
register.filter('qstring_page', qstring_page)
register.filter('qstring_sort', qstring_sort)